...
function MoviePage({
  ...
  async function handleLike(reviewObj) {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_RAILS_URL}/reviews/${reviewObj.id}`,
      {
        method: "PATCH",
        headers: {
  ...
  useEffect(() => {
    const fetchMovieById = async () => {
      const response = await fetch(
        `${process.env.REACT_APP_RAILS_URL}/movies/${id}`
      );
      const jsonify = await response.json();
  ...
  const { title, image, genre, runtime, tagline, release_year, overview } =
    movieToDisplay;
  ...
  async function handleFollowOtherUser(userToFollow) {
    ...
    const response = await fetch(
      `${process.env.REACT_APP_RAILS_URL}/friendships`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const allReviews = movieReviews.map((review) => {
    return (
      <div key={review.id} className="all-movie-reviews">
        ...
        <img
          className="movie-page-author-image"
        ...
        <div className="all-movie-review-details">
          ...
          {canLeaveReview ? (
            <button
              ...
              onClick={(e) => handleFollowOtherUser(review.author_object)}
            >
              Follow User
            </button>
          ) : (
          ...
          <button onClick={(e) => handleLike(review)} className="like-button">
            ❤️ Like Review
          </button>
  ...
  return (
    <div className="movie-show-page">
      ...
      <button onClick={handleCanLeaveReview} id="myBtn">
        Leave Review
      </button>
      {canLeaveReview ? (
        <div className="modal-content">
          <div className="modal-header">
            <span className="close" onClick={handleCanLeaveReview}>
              &times;
            </span>
